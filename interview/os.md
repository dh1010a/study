...
<details>
  <summary>만약 전체 메모리는 넉넉하다고 가정해도, heap 영역을 과하게 크게 할당하면 어떤 일이 발생할까요?</summary>
  <ul>
    <li> heap 영역이 과하게 크면 가비지 컬렉션 같은 메모리 관리 시스템이 스캔해야하는 범위가 넓어져 시간이 오래걸리고 수행 시간이 오래걸리게 되며, OOM 발생시 해결이 어렵다는 문제가 있습니다.</li>
  </ul>
</details>
...
<details>
  ...
  <ul>
    <li> 시분할 시스템에서는 한 프로세스가 매우 짧은 시간동안 CPU를 점유하여 작업을 진행하다가 다른 프로세스에 점유권을 넘기고, 나중에 다시 넘겨받아 실행하게 됩니다. 이때 이전에 어디까지 명령을 수행했고 register들에는 어떤 값이 있었는지에 대한 정보가 필요합니다. 프로세스가 어떤 상태로 실행되고 있었는지에 대한 총체적인 정보를 라고 합니다. 이 들은 pcb에 저장되게 됩니다. </li>
  </ul>
</details>
...
<details>
  ...
  <ul>
    <li> 스케줄링은 운영체제가 CPU를 여러 프로세스와 스레드에 어떤 순서로 배분할지 결정하는 매커니즘입니다. CPU는 한 순간에 하나의 명령만 실행할 수 있기 때문에, 스케줄러 작업을 통해 작업을 빠르게 전환하며 마치 동시에 실행되는 것처럼 보이게 합니다. 스케줄링 방식에는 크게 비선점과 선점이 있습니다. 비선점은 프로세스가 CPU를 스스로 놓을 때까지 실행되는것이고, 선점은 OS가 타임슬라이스나 우선순위를 기반으로 CPU를 강제로 회수할 수 있는 방식입니다. 현대 운영체제는 대부분 선점 방식을 사용합니다.</li>
    <li> 선점 알고리즘에는 라운드로빈, 우선순위 스케줄링 등이 있습니다. 비선점형 알고리즘에는 가장 먼저 도착순서대로 할당하거나, Shortest Jop First 등이 있습니다.</li>
  </ul>
</details>
...
<details>
  <summary>왜 컨텍스트 스위칭이 스레드보다 프로세스가 느린가요?</summary>
  <ul>
    <li> 멀티 프로세스에서 컨텍스트 스위칭이 느린 이유는 저장하고 복원해야할 상태가 스레드에 비해 훨씬 많기 때문입니다. 프로세스는 독립된 주소 공간을 가지고 있으므로 스위칭 시 CPU 레지스터 뿐만 아니라 페이지 테이블 전체가 교체되고 이때 캐시 TLB 플러스기 발생해 성능이 크게 감소합니다. 또한 프로세스 스위칭은 유저 모드와 커널모드 간의 전환이 필요하고 캐시 정보가 무효화 되기도 합니다. </li>
    <li> 반면 멀티 프레드는 같은 프로세스의 주소 공간을 공유하기 때문에 변경 정보가 적어 주소 공간 체계나 캐시와 같은 정보들이 초기화되지 않아 훨씬 적응 비용이 소모됩니다. </li>
  </ul>
</details>

<details>
  <summary>TLB는 정확히 뭔가요?</summary>
  <ul>
    <li> CPU가 가상주소를 물리 주소로의 변환을 빠르게 하기 위해 사용하는 고속 캐시입니다. 운영체제는 가상 메모리를 사용하기 때문에 CPU는 메모리 접근 시 반드시 주소 변환 과정을 거쳐야 합니다. 그 과정은 페이지 테이블 조회로 일어나는데, 메모리에 있는 데이터 구조이기 때문에 접근이 느립니다. 그래서 CPU는 자주 쓰는 변환 결과를 TLB라는 고속 캐시에 저장해둡니다. </li>
    <li> 다른 프로세스 스위칭시 TLB를 모두 비웠기 때문에 다량의 캐시 미스가 발생하고, 페이지 테이블 메모리에 접근해 캐시를 채우는 과정이 반복되므로 시간이 오래 소요됩니다.</li>
  </ul>
</details>
...
<details>
  <summary>CPU가 메모리에 있는 데이터를 어떻게 접근하나요?</summary>
  <ul>
    <li> CPU가 메모리에 접근할떄 가상 주소를 생성하고 이를 MMU가 TLB와 페이지 테이블을 이용해 물리 주소로 변환합니다. 물리 주소가 결정되면 CPU는 먼저 L1->L2 캐시를 조회하고 없다면 메모리 접근 요청이 메모리 컨트롤러로 전달됩니다. 메모리 컨트롤러는 메모리 버스를 통해 메모리에 접근해 데이터를 읽어오고 그것을 l2 l1 캐시에 적재합니다.</li>
  </ul>
</details>


<details>
  <summary>시스템 버스에 대해 설명해주세요</summary>
  <ul>
    <li> 시스템 버스는 CPU-메모리-I/O를 연결하는 통신 구조입니다. 데이터,주소,제어 버스로 구성되어 각각 데이터 전달(메모리가 실제로 읽어온 값, CPU 연산 값), 주소 지정(CPU가 어떤 주소의 메모리에 접근할 것인가 를 알려주는 통로), 제어 신호(read/write 신호, 인터럽트 신호 등) 전송을 담당합니다.</li>
    <li> 다양한 장치가 버스를 사용하기 때문에 누가 버스를 사용하는지 결정해줘야 하는데 이것을 버스 아비터(Bus Arbiter)라고 합니다.</li>
  </ul>
</details>

<details>
  <summary>버스 아비터가 뭔가요?</summary>
  <ul>
    <li> 장치들이 동일한 시스템 버스를 사용할 때 누가 버스를 사용할지 결정하는 하드웨어 장치입니다. 라운드로빈이나 우선순위에 따라 버스 이용권한을 부여합니다.</li>
  </ul>
</details>

<details>
  <summary>CPU가 메모리와 통신하는 과정은 어떻게 되나요?</summary>
  <ul>
    <li> 먼저 주소 버스를 통해 특정 주소에 접근하고 싶다는 요청을 보냅니다. 제어버스는 읽기인지 쓰기인지 신호를 보내고, 메모리 허가등의 제어 신호를 보냅니다. 최종적으로 데이터 버스를 통해 데이터를 주고받게 됩니다.</li>
  </ul>
</details>
...
<details>
  <summary> 논리적 주소를 왜 사용하는건가요?</summary>
  <ul>
    <li> 프로세스 보호, 독립된 주소 공간 제공, 효율적인 메모리 활용, 가상 메모리 구현 떄문입니다. 프로그램은 물리적 주소를 모르는 채로 자신만의 논리 주소를 사용하고, 실제 주소로 변환하는 일은 MMU나 페이지 테이블이 수행합니다. 이 구조가 있어 프로세스 끼리의 물리적 주소 침범이 방지되는 채로 프로그램을 실행할 수 있으며, 스왑을 통해 물리 메모리 보다 큰 주소공간을 제공할 수 있습니다. 물리적으로 연속되지 않게 사용할 수 있어 메모리를 더 효율적으로 사용할 수 있게 됩니다.</li>
  </ul>
</details>
...
<details>
  ...
  <ul>
    ...
    <li> 이를 방지하기 위해서는 프로세스를 줄여 멀티 프로그래밍 정도를 낮추거나 working set model을 적용해 필요한 페이지 집합을 메모리에 유지하거나, 물리메모리를 증설하는등으로 해결할 수 있습니다. 또한 운영체제는 이 현상을 방지하기 위해 working set 기반의 메모리 관리 정책과 page fault 빈도 제어(fault가 많으면 프레임을 더 주고, 적으면 회수) 기법을 사용합니다.</li>
  </ul>
</details>
